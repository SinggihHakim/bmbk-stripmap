...
class ExcelImporter
{
        ...
        if ($ext === 'xlsx') {
            ...
            if ($zip->open($filePath) === true) {
                ...
                foreach ($sheetMapping as $sheetName => $xmlPath) {
                    ...
                    // Abaikan sheet yang ditandai non-aktif / silang (diakhiri dengan huruf X)
                    if (preg_match('/\bX\s*$/i', $trimmedName)) {
                        continue;
                    }

                    $lowerName = strtolower($trimmedName);
                    if (str_contains($lowerName, 'aspal') && !str_contains($lowerName, 'non')) {
                        if ($aspalSheet === null || (!str_contains($lowerName, 'rekap') && str_contains(strtolower($aspalSheetName ?? ''), 'rekap'))) {
                            $aspalSheet = $xmlPath;
                            $aspalSheetName = $sheetName;
                        }
                    } elseif (str_contains($lowerName, 'beton')) {
                        if ($betonSheet === null || (!str_contains($lowerName, 'rekap') && str_contains(strtolower($betonSheetName ?? ''), 'rekap'))) {
                            $betonSheet = $xmlPath;
                            $betonSheetName = $sheetName;
                        }
                    } elseif (str_contains($lowerName, 'non') || str_contains($lowerName, 'agregat') || str_contains($lowerName, 'tanah')) {
                        if ($nonAspalSheet === null || (!str_contains($lowerName, 'rekap') && str_contains(strtolower($nonAspalSheetName ?? ''), 'rekap'))) {
                            $nonAspalSheet = $xmlPath;
                            $nonAspalSheetName = $sheetName;
                        }
                    }
                }
    ...
    private function processDataRows(array $rows): array
    {
        ...
        // Deteksi header dari 30 baris pertama untuk mengakomodasi file Excel dengan judul/logo panjang di atas
        $maxHeaderRows = min(30, count($rows));
        ...
        foreach ($rows as $rowIndex => $row) {
            ...
            // Parsing Panjang (SK)
            $panjangVal = isset($colMap['panjang']) ? $this->parseFloatVal($row[$colMap['panjang']] ?? '0') : 0;
            $panjangMeter = round(($panjangVal > 500 ? $panjangVal : $panjangVal * 1000), 2);
            ...
            $rigidVal   = isset($colMap['rigid']) ? $this->parseFloatVal($row[$colMap['rigid']] ?? '0') : 0;
            $aspalVal   = isset($colMap['aspal']) ? $this->parseFloatVal($row[$colMap['aspal']] ?? '0') : 0;
            $telfordVal = isset($colMap['telford']) ? $this->parseFloatVal($row[$colMap['telford']] ?? '0') : 0;
            $tanahVal   = isset($colMap['tanah']) ? $this->parseFloatVal($row[$colMap['tanah']] ?? '0') : 0;

            $sumPk = $rigidVal + $aspalVal + $telfordVal + $tanahVal;
            $multPk = ($sumPk > 500) ? 1.0 : 1000.0;

            $rigidM   = round($rigidVal * $multPk, 2);
            $aspalM   = round($aspalVal * $multPk, 2);
            $agregatM = round(($telfordVal + $tanahVal) * $multPk, 2);
            $totalPerkerasanM = $rigidM + $aspalM + $agregatM;

            // Parsing Kondisi (Km -> meter)
            $baikVal        = isset($colMap['baik']) ? $this->parseFloatVal($row[$colMap['baik']] ?? '0') : 0;
            $sedangVal      = isset($colMap['sedang']) ? $this->parseFloatVal($row[$colMap['sedang']] ?? '0') : 0;
            $rusakRinganVal = isset($colMap['rusak_ringan']) ? $this->parseFloatVal($row[$colMap['rusak_ringan']] ?? '0') : 0;
            $rusakBeratVal  = isset($colMap['rusak_berat']) ? $this->parseFloatVal($row[$colMap['rusak_berat']] ?? '0') : 0;

            $sumCond = $baikVal + $sedangVal + $rusakRinganVal + $rusakBeratVal;
            $multCond = ($sumCond > 500) ? 1.0 : 1000.0;

            $baikM   = round($baikVal * $multCond, 2);
            $sedangM = round($sedangVal * $multCond, 2);
            $rrM     = round($rusakRinganVal * $multCond, 2);
            $rbM     = round($rusakBeratVal * $multCond, 2);
            $totalKondisiM = $baikM + $sedangM + $rrM + $rbM;

            $finalPanjangMeter = max($panjangMeter, $totalKondisiM, $totalPerkerasanM);
    ...
    private function importDetailedSurvey(ZipArchive $zip, ?string $aspalSheet, ?string $betonSheet, ?string $nonAspalSheet): array
    {
        ...
        foreach ($sheetsToProcess as $type => $info) {
            ...
            $colMap = [];
            // Pindai baris 15-25 (mengabaikan baris judul di atas seperti "BACK - UP DATA SURVEY...")
            for ($i = 15; $i <= 25; $i++) {
                ...
                foreach ($rows[$i] as $idx => $val) {
                    ...
                    if ($clean === 'dari' || $clean === 'dari (m)' || $clean === 'dari ( m )' || str_contains($clean, 'sta awal') || str_contains($clean, 'sta. awal')) {
                        if (!isset($colMap['sta_awal'])) $colMap['sta_awal'] = $idx;
                    } elseif ($clean === 'ke' || $clean === 'ke (m)' || $clean === 'ke ( m )' || str_contains($clean, 'sta akhir') || str_contains($clean, 'sta. akhir')) {
                        if (!isset($colMap['sta_akhir'])) $colMap['sta_akhir'] = $idx;
                    } elseif (str_contains($clean, 'lebar')) {
                        if (!isset($colMap['lebar'])) $colMap['lebar'] = $idx;
                    } elseif (preg_match('/^kondisi(\s+\d{4}|\s+jalan|\s+2[0-9]{3})?$/i', $clean) || $clean === 'status' || $clean === 'mutu' || $clean === 'b/s/rr/rb') {
                        // Selalu update sehingga kolom tahun terbaru (misal "KONDISI 2025") yang menang
                        $colMap['kondisi'] = $idx;
                    }
                }
            }

            $hasPerColKondisi = isset($colMap['col_baik']) || isset($colMap['col_sedang']) || isset($colMap['col_rr']) || isset($colMap['col_rb']);
            $hasSingleKondisi = isset($colMap['kondisi']);

            if (!isset($colMap['sta_awal']) || !isset($colMap['sta_akhir'])) {
                continue;
            }

            foreach ($rows as $rData) {
                $staAwalVal  = isset($rData[$colMap['sta_awal']]) ? trim((string)$rData[$colMap['sta_awal']]) : '';
                $staAkhirVal = isset($rData[$colMap['sta_akhir']]) ? trim((string)$rData[$colMap['sta_akhir']]) : '';

                if ($staAwalVal === '' || $staAkhirVal === '') continue;
                ...
                if ($hasPerColKondisi) {
                    $baikVal   = isset($colMap['col_baik'])   ? $this->parseFloatVal($rData[$colMap['col_baik']]   ?? '0') : 0;
                    $sedangVal = isset($colMap['col_sedang']) ? $this->parseFloatVal($rData[$colMap['col_sedang']] ?? '0') : 0;
                    $rrVal     = isset($colMap['col_rr'])     ? $this->parseFloatVal($rData[$colMap['col_rr']]     ?? '0') : 0;
                    $rbVal     = isset($colMap['col_rb'])     ? $this->parseFloatVal($rData[$colMap['col_rb']]     ?? '0') : 0;
                    $sumCond   = $baikVal + $sedangVal + $rrVal + $rbVal;
                    if ($sumCond > 0) {
                        $baik   = $baikVal;
                        $sedang = $sedangVal;
                        ...
                        if ($type === 'rigid') $baik = $panjang;
                        elseif ($type === 'aspal') $sedang = $panjang;
                        else $rr = $panjang;
                    }
                } elseif ($hasSingleKondisi) {
                    $kondisiVal = isset($rData[$colMap['kondisi']]) ? trim((string)$rData[$colMap['kondisi']]) : '';
                    $cond = strtoupper($kondisiVal);
                    if ($cond === 'B' || $cond === '1' || str_contains($cond, 'BAIK')) {
                        $baik = $panjang;
                    } elseif ($cond === 'S' || $cond === '2' || str_contains($cond, 'SEDANG')) {
                        $sedang = $panjang;
                    } elseif ($cond === 'RR' || $cond === 'R' || $cond === '3' || str_contains($cond, 'RINGAN')) {
                        $rr = $panjang;
                    } elseif ($cond === 'RB' || $cond === '4' || str_contains($cond, 'BERAT')) {
                        $rb = $panjang;
                    } else {
                        if ($type === 'rigid') $baik = $panjang;
                        elseif ($type === 'aspal') $sedang = $panjang;
