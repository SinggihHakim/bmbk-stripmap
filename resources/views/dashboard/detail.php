...
<div x-data="dashboardDetailTable(<?= htmlspecialchars(json_encode($ruasDetailJsonData), ENT_QUOTES, 'UTF-8') ?>, '<?= $selectedKondisi ?>')" class="space-y-6">
    ...
    <!-- Filters, Search & Table Panel Section -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden" x-data="{ isSearchOpen: true }">
        <div class="px-5 py-4 border-b border-gray-200 bg-gray-50/70 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 cursor-pointer select-none" @click="isSearchOpen = !isSearchOpen">
            <div>
                ...
                <p class="text-xs text-gray-500">Filter, urutkan, dan lihat data ruas jalan secara langsung.</p>
            </div>
            <button type="button"
                    ...
                    <div class="w-full md:w-auto">
                        <button type="button" 
                            ...
                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v18"/>
                            </svg>
                            <span x-text="sortOrder === 'desc' ? 'Terpanjang \u2192 Terpendek' : 'Terpendek \u2192 Terpanjang'"></span>
                        </button>
                    </div>
...
function dashboardDetailTable(initialData, initialKondisi) {
    return {
        ...
        filteredRuas() {
            ...
            // Sorting: Default Presentase/Panjang Kondisi Terpilih
            result.sort((a, b) => {
                const pctA = parseFloat(this.getKondisiPctOfRuas(a));
                const pctB = parseFloat(this.getKondisiPctOfRuas(b));
                if (pctA !== pctB) {
